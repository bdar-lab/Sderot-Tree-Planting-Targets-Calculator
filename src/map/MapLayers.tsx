...
import { FILTER_LAYER_MAPPINGS, SELECTED_LAYER_TITLE } from './layers'
import { STANDALONE_LAYER_ICONS } from './layer-icons'

// The grey "Selected streets" base is an always-on canvas the user should
// never toggle off — hiding it from the panel makes the highlight overlay
// (renamed to just "Selected streets" for the panel) the only entry the
// user interacts with for street selection.
const HIDE_FROM_PANEL = new Set<string>([SELECTED_LAYER_TITLE])
  ...
  useEffect(() => {
    const collect = () => {
      ...
      for (const layer of layers as any[]) {
        ...
        if (HIDE_FROM_PANEL.has(canonical)) continue
